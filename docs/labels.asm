...
save_command_trap:
	...
	call load_message_pointer;							// call UnoDOS file save handler
...
handle_unodos_load:
	push de;							// save filename pointer
	call load_message_pointer;							// call UnoDOS file load handler
	pop de;								// restore filename pointer
...
handle_disk_status:
	...
	call find_handle_by_index;			// call system routine
	ccf;								// complement carry flag
...
directory_update_processing:
directory_update_processing_loop:
	djnz directory_update_processing_loop;	// loop back to file flush if B register not zero
...
conditional_jump_loop:
	jr z, conditional_jump_loop;		// jump if condition met
...
save_current_address:
	ld (conditional_jump_loop), hl;		// save current address
...
; Function: Load alternate message pointer
...
call_memory_mgmt:
	...
	exx;								// switch back to main register set
	jr memory_allocation_return;			// loop for memory block processing
...
directory_entry_attribute_mgmt:
	...
	ld (hl), a;							// store in directory buffer
	call directory_sector_write;		// call directory sector access
	pop de;								// restore directory entry pointer
...
; // file write operation error handler
write_error_handler:
